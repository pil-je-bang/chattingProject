...
void add_client() {
    ...
    while (1) {
        ...
        if (strcmp(buf, "1") == 0) {
            ...
            cout << "소켓리스트 크기 : "<<sck_list.size() << endl;
            ...
            int client_count_real = 0;

            for (int i = 0; i < client_count; i++) {
                if (sck_list[i].is_it_connect)
                    client_count_real++;
            }
            cout << "[공지] 현재 접속자 수 : " << client_count_real << "명" << endl;
...
void del_client(int idx) {
    ...
    sck_list[idx].is_it_connect = false;
    //client_count--;
}