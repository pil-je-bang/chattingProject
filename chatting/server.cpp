...
void add_client(int ti) {
    ...
    while (1) {
        ...
        char buf[MAX_SIZE] = { };
        char buf2[MAX_SIZE] = { };
        recv(new_client.sck, buf, MAX_SIZE, 0); // 몇번 메뉴 실행 할지?
        ...
        if (strcmp(buf, "1") == 0) { // 로그인
            ...
            cout << "buf2 :" << buf2;
            recv(new_client.sck, buf2, sizeof("1"), 0); //채팅시작 //메인메뉴 채팅방입장 받아오기
            if (strcmp(buf2, "1") == 0) { // 채팅방 입장!!!
            ...
            else if (strcmp(buf2, "5") == 0) { // 로그아웃
                string msg;
                msg = "[공지] " + new_client.user + " 님이 로그아웃했습니다.";
                cout << msg << endl;
                //del_client(idx); // 클라이언트 삭제
                logoutSoket(new_client.user);
            }
        }
...
void send_dm_msg(const char* msg, int i, string user) { //dm
    string q = "/dm" + user + "의 귓속말:";
    for (int i = 0; i < sck_list.size(); i++) { // 접속해 있는 모든 client에게 메시지 전송
    ...
    while (1) {
        ...
        if (recv(sck, buf, MAX_SIZE, 0) > 0) { // 오류가 발생하지 않으면 recv는 수신된 바이트 수를 반환. 0보다 크다는 것은 메시지가 왔다는 것
            ...
            if (findsubstr(t, 3) == "/dm") {
                ...
                cout << dm_id << endl;
                send_dm_msg(dm_message.c_str(), sck_list.size(), dm_id);
                SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 10);
                cout << user << "가 " << dm_id << "에게 " << dm_message;
            }
...
int logoutSoket(string user) {
    ...
    for (int i = 0; i < sck_list.size(); i++) {
        if (sck_list[i].user == user) {
            ti = sck_list[i].ti;
            sck_list.erase(sck_list.begin() + i);
            return ti;
        }
    }
    return -1;
